...
def list_orders(db: Session = Depends(database.get_db)):
    return controllers.orders.get_orders(db)

@router.get("/{order_id}", response_model=schemas.Order)
def read_order(order_id: int, db: Session = Depends(database.get_db)):
    db_order = controllers.orders.get_order(db, order_id)
    if not db_order:
